...
contract VORZ is Context, IMRC20, Ownable {
    ...
    uint256 private _buyBackUpperLimit = 1500 * 10**18;
    uint256 private _buyBackLowerLimit = 150 * 10**18;
    uint256 private max_MATIC = 10000000000 * 10**18;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(!(_msgSender() == vorzShop && recipient == quickswapV2Pair), "VorzShop cannot sell");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(!(sender == vorzShop && recipient == quickswapV2Pair), "VorzShop cannot sell");
    ...
    function getEquivalentInToken(uint256 amountOfMATIC) public view returns (uint equivalentInToken) {
        (uint vorzInPool, uint maticInPool) = getReservesInPool();
        return ((amountOfMATIC.mul(vorzInPool)).div(maticInPool));
    }
    ...
    function getMarketingAddress() public view returns (address marketingAddress) {
        ...
        require(getEquivalentInMATIC(tokenAmount) < max_MATIC, "VorzShop cannot sell amounts higher than 10000000000 MATIC");
    ...
    function setBuybackUpperLimit(uint256 buyBackUpperLimit) external onlyVorzAdmin {
        require(buyBackUpperLimit > 150 * 10**18, "Buy-back upper limit must be higher than 150MATIC");
    ...
    function setBuybackLowerLimit(uint256 buyBackLowerLimit) external onlyVorzAdmin {
        require(buyBackLowerLimit <= 150 * 10**18, "Buy-back lower limit must be lower than 150MATIC");
    ...
    function rescueTokens(address _tokenAddr, uint256 amount) external onlyOwner{
        IMRC20(_tokenAddr).transfer(msg.sender, amount);
    }
